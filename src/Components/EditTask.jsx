...
const EditTask = ({ onClose, task }) => {
    ...
    useEffect(() => {
        const fetchTask = async () => {
            if (task && task._id) {
                try {
                    const response = await axios.get(`http://localhost:3002/tasks/${task._id}`);
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await axios.patch(`http://localhost:3002/tasks/${task._id}`, updatedTask);
    ...
    return (
        <Box display="flex" justifyContent="center">
            <Box sx={{ border: '2px solid #DCDCDC', width: '640px', overflow: 'hidden' }}>
                <Box sx={{ textAlign: 'center', padding: '15px', borderBottom: '1.5px solid #DCDCDC', backgroundColor: '#F7F7F7' }}>
                    <Typography variant="h6" sx={{ fontWeight: 'bold', fontSize: '18px' }}>Edit Task</Typography>
                </Box>
                <Box sx={{ padding: '15px' }}>
                    <form>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '15px' }}>
                            <Button variant="contained" sx={{ backgroundColor: '#ffeeba', color: '#000', borderRadius: '4px', marginRight: '10px' }} onClick={handleCancel}>
                                Cancel
                            </Button>
                            <Button variant="contained" sx={{ backgroundColor: '#4CAF50', color: 'white', borderRadius: '4px' }} onClick={handleSave}>
                                Save 
                            </Button>
                        </div>
                    </form>
